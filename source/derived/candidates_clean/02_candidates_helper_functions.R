load_raw_data <- function(path) {
  ...
  # Helper functions for cleaning and validating candidate data
  
  #' Load and validate raw candidate data
  #' @param path Path to raw data file
  #' @return Validated raw dataframe
  
  if (!file.exists(path)) {
    log_error(sprintf("Input file not found: %s", path))
  }
  ...
  df <- read_csv(
    path,
    col_types = cols(
      race_id = col_character(),
      year = col_integer(),
      race = col_character(),
      race_type = col_character(),
      primary = col_logical(),
      statedistrict = col_character(),
      name = col_character(),
      party = col_character(),
      status = col_character(),
      unopposed = col_logical(),
      votes = col_number(),
      totalvotes = col_number(),
      win = col_logical(),
      share_male = col_double(),
      share_white = col_double(),
      share_black = col_double(),
      share_hisp = col_double()
    )
  )
